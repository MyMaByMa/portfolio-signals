...
def guru_mix_scores(tech_row, fund_row, weights):
    ...
    comp = dict(
        buffett=buffett, graham=graham, greenblatt=greenblatt,
...
def read_positions(path="positions.csv"):
    ...
    - oddělovače čárka/semicolon/mezera/tab
...
def main():
    ...
    e_days = (
        cfg.get("signals", {}).get("earnings_blackout_days") or
        cfg.get("watchlist_rules", {}).get("earnings_blackout_days") or
        1
    )

    for tkr, row in tech.iterrows():
        fund = compute_fundamentals_row(tkr)
        name = fund.get("name") or tkr
        gm, comp = guru_mix_scores(row, fund, weights)
        e_blk = earnings_blackout(tkr, e_days)

        # shrnutí „why“
        parts = []
        parts.append("Close>SMA50" if row["close"] > row["sma50"] else "Close<SMA50")
        try:
            parts.append(f"RSI={row['rsi']:.1f}")
        except Exception:
            pass
        try:
            if not np.isnan(row["dist_52w_hi"]):
                parts.append(f"{row['dist_52w_hi']*100:.1f}% pod 52W high")
        except Exception:
            pass
        why = "; ".join(parts)
        ...
        rows.append({
            "name": name,
            "ticker": tkr,
            "bucket": bucket, "signal": classify_signal(dict(
    ...
    out = pd.DataFrame(rows).sort_values(
        ["bucket", "signal", "guru_mix"], ascending=[True, True, False]
    )
    ...
    # počítadlo g20 (0–20) z guru_mix
    if "guru_mix" in out.columns:
        out["gm20"] = np.clip(out["guru_mix"] / 5.0, 0, 20)
    else:
        out["gm20"] = 0.0

    # rámec pro alokace + kandidáti
    df = out[["ticker", "signal", "guru_mix", "close", "gm20"]].copy()
...
for tkr, t_sh in tgt_sh.items():
    ...
    # ulož CSV s plánem a cíli (i kdyby prázdné)
